import numpy as np
import pandas as pd
Amtrak = pd.read_csv("file:///F:/vinod sir class(Python)/forecasting/Amtrak.csv")
...
Amtrak["t_squared"] = Amtrak["t"]*Amtrak["t"]
Amtrak.columns
...
help("statsmodels")
linear_model = smf.ols(formula="Ridership~t",data=Train).fit()
...
Exp = smf.ols(formula='log_Rider~t',data=Train).fit()
...
predict_data = pd.read_csv("file:///F:/pythone codes/forecasting/Predict_new.csv")
model_full = smf.ols('Ridership~t+t_squared+Jan+Feb+Mar+Apr+May+Jun+Jul+Aug+Sep+Oct+Nov',data=Amtrak).fit()
